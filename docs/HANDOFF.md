...
Poslední update: **2026-06-24** · repo `Anamax443/BC_Telemetry`.
Push #66 (Naposledy s časem): sloupec **NAPOSLEDY** v Aktivitě (a souhrn v Uživatelích) ukazuje **datum + čas**, ne jen datum. Příčina: `LastUsed = MAX(DateKey)` četl z denního agregátu `BCPageDaily` (typ `DATE` → čas zahozen) a `Invoke-Sql` navíc usekává každý `DateTime` na `yyyy-MM-dd`. Fix **jen ve snapshotu** (`Export-DashboardSnapshot.ps1`, query `userActivity`): `LastUsed` se bere jako `MAX(Timestamp)` z raw `dbo.BCPageLog` (covering index `IX_BCPageLog_UserName_Timestamp`) a vrací přes `CONVERT(varchar(19),…,120)` jako string (jinak by ho `Invoke-Sql` usekl); `COALESCE` na `CAST(DateKey AS datetime2)` (půlnoc) pro řádky mimo 6měs. retenci raw. **Žádná změna `index.html`** (`fdate` jen `-`→`.`; filtr data parsuje vedoucí datum; řazení lexikografické = chronologické), **žádné SQL DDL**, **bez restartu služby** — jen redeploy snapshotu + regenerace `data.json` (/refresh). Čas je UTC (jako zbytek dashboardu). ⏳ **Deploy čeká na autorizaci** (classifier blokuje zápis na produkční share `\\10.8.2.225`).
