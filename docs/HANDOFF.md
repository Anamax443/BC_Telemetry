...
Push #67 (e-mailové notifikace + periodická hlášení): **stav a alerty e-mailem** přes **M365 Direct Send** (`axima-cz.mail.protection.outlook.com:25`, STARTTLS, bez auth, From `bc-telemetry@axima.cz`, příjemci jen `@axima.cz`) — zrcadlí ITDashboard. Odesílá **wrapper (svc) po snapshotu** přes `scripts/Send-BCTelemetryNotification.ps1`; konfigurace `ops/notify.json` (`GET/PUT /notify-config`), stav last-sent/last-status `ops/notify-state.json`. **Alerty:** selhání importu (`exit≠0`/`Import ROLLBACK` v denním logu), **zastaralý modul A/B** (nejnovější `LastUsed`/`ChangedAt` starší než `staleHours`, def. 24 h), **expirace SP secretu** (`secretExpiry` 2028-06-07, var `secretWarnDays` předem). **Sync drift se NEHLÍDÁ** — sync je ~5 % záměrně (cloud drží vše, lokál výřez), proto se sleduje **čerstvost**, ne total %. **Perioda** volitelná: `daily` (souhrn 1×/den + nový problém) / `perRun` / `onChange`. Předmět `[OK]|[CHYBA] [RUČNĚ?] BC Telemetrie — stav RRRR-MM-DD`. **UI** ⚙ Nastavení → „E-mailové notifikace" (vč. tlačítka **✉ Poslat testovací e-mail** → `POST /notify-test`, pošle `[RUČNĚ]` hned přes web/LocalSystem). ✅ **Ověřeno živě 2026-06-24**: test e-mail odeslán a doručen na mtrnka@axima.cz; defaultně **zapnuto** (`enabled=true`, denní). ⚠ `server.js` = restart služby (proveden).
